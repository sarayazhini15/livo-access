...
function buildSpeech(r) {
  ...
  if (r.verified) {
    parts.push("Bill verified. The calculations are correct.");
  } else {
    parts.push("Check bill. There is a problem.");
    (r.issues || []).forEach((i) => parts.push(i.replace(/rupees/g, "rupees ")));
  }
  return parts.join(" ");
}
...
export default function BillChecker() {
  ...
  const handleFile = async (e) => {
    ...
    setResult(null);
    setLoading(true);
    speak("Reading your bill. Please wait.");
    try {
  ...
  return (
    <div className="space-y-8" data-testid="bill-checker-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <ActionButton icon={Camera} label="Take Photo" testid="bill-take-photo-btn" variant="primary" onClick={() => cameraRef.current?.click()} />
      ...
      <section aria-label="Bill analysis result" className="space-y-4" aria-live="polite">
        <div className="flex items-center justify-between">
          ...
          {result && (
            <button
              onClick={() => speak(buildSpeech(result))}
              data-testid="bill-replay-btn"
        ...
        {result && !loading && (
          <div className="space-y-4" data-testid="bill-result">
            <div
              ...
              className={`flex items-center gap-4 border-4 p-5 ${
                result.verified ? "bg-primary text-black border-primary" : "bg-white text-black border-white"
              }`}
            >
              {result.verified ? (
                <CheckCircle2 size={48} strokeWidth={2.5} aria-hidden="true" />
              ) : (
                <AlertTriangle size={48} strokeWidth={2.5} aria-hidden="true" />
              )}
              <span className="font-heading text-3xl sm:text-4xl font-black uppercase tracking-wide">
                {result.status}
              </span>
            </div>
                ...
                <ul className="space-y-2 list-none">
                  {result.issues.map((issue, i) => (
                    <li key={i} className="flex items-start gap-3 text-lg sm:text-xl text-white">
                      ...
                      <span>{issue.replace(/rupees/g, "₹")}</span>
                    </li>
                  ))}
                </ul>
              </div>
